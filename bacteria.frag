...
vec3 extra_cheap_atmosphere(vec3 raydir, vec3 sundir) {
    ...
    float special_trick2 = 1.0 / (sundir.y * 8.0 + 1.0);
    float raysundt = pow((0.5 + 0.5 * dot(sundir, raydir)), 2.0);
    float sundt = pow(max(0.0, 0.5 + 0.5 * dot(sundir, raydir)), 8.0);
    float mymie = sundt * special_trick * 0.1;
    vec3 suncolor = mix(vec3(1.0), max(vec3(0.0), vec3(1.0) - vec3(5.5, 13.0, 22.4) / 22.4), special_trick2);
    vec3 bluesky = vec3(5.5, 13.0, 22.4) / 22.4 * suncolor;
    vec3 bluesky2 = max(vec3(0.0), bluesky - vec3(5.5, 13.0, 22.4) * 0.002 * (special_trick + -6.0 * sundir.y * sundir.y));
    bluesky2 *= special_trick * (0.1 + raysundt * 0.44);
    return 0.5 * bluesky2 * (1.0 + 1.0 * pow(1.0 - raydir.y, 3.0)) * max(0.1, 1.0 - 1.0 / exp(raydir.y * 10.0)) + mymie * suncolor;
}
vec3 getatm(vec3 ray) {
    return extra_cheap_atmosphere(ray, normalize(vec3(1.0, 0.1, 1.0))) * 0.5;

}

float sun(vec3 ray) {
    vec3 sd = normalize(vec3(1.0, 0.1, 1.0));
...
void main(void) {
    ...
    R.y = abs(R.y);
    R = mix(N, R, 1.0 / (dist * dist * 0.0001 + 1.0));
